<nav id="main-navbar" class="fixed w-full z-50 top-0 transition-all duration-300 bg-transparent">
    <div class="container mx-auto px-6 h-28 flex justify-between items-center relative">
        ...
        <a href="{{ route('home') }}" class="flex items-center justify-center z-50 h-full overflow-visible">
            <img
        ...
        <div class="hidden md:flex space-x-6 items-center">
            <a href="{{ route('home') }}"
                class="nav-link inline-block {{ request()->routeIs('home') ? 'text-brand dark:text-brand-light border-b-2 border-brand' : 'text-white' }} font-bold hover:text-brand dark:hover:text-brand-light transition pb-1">
                Início
            </a>

            <a href="{{ route('servicos') }}"
                class="nav-link inline-block {{ request()->routeIs('servicos') ? 'text-brand dark:text-brand-light border-b-2 border-brand' : 'text-white' }} font-bold hover:text-brand dark:hover:text-brand-light transition pb-1">
                Serviços e Produtos
            </a>

            <a href="{{ route('acondicionamento') }}"
                class="nav-link inline-block {{ request()->routeIs('acondicionamento') ? 'text-brand dark:text-brand-light border-b-2 border-brand' : 'text-white' }} font-bold hover:text-brand dark:hover:text-brand-light transition pb-1">
                Acondicionamento
            </a>

            <a href="{{ url('/transportes') }}"
                class="nav-link inline-block {{ request()->is('transportes') ? 'text-brand dark:text-brand-light border-b-2 border-brand' : 'text-white' }} font-bold hover:text-brand dark:hover:text-brand-light transition pb-1">
                Transportes / Locação
            </a>
            ...
            <div class="flex items-center pl-6 ml-2 gap-3">
                <button onclick="setTheme('light')" class="theme-button text-white/70 hover:text-yellow-400 transition"
                    title="Modo Claro">
                    <i class="fas fa-sun text-xl"></i>
                </button>

                <button onclick="setTheme('dark')" class="theme-button text-white/70 hover:text-brand-light transition"
                    title="Modo Escuro">
    ...
    <div id="mobile-menu"
        ...
        <div class="flex flex-col px-6 py-6 space-y-5 text-center">
            ...
            <a href="{{ route('servicos') }}" onclick="toggleMobileMenu()"
                class="text-gray-900 dark:text-white font-bold text-lg hover:text-brand transition">
                Serviços e Produtos
            </a>

            <a href="{{ route('acondicionamento') }}" onclick="toggleMobileMenu()"
                class="text-gray-900 dark:text-white font-bold text-lg hover:text-brand transition">
                Acondicionamento
            </a>

            <a href="{{ url('/transportes') }}" onclick="toggleMobileMenu()"
                class="text-gray-900 dark:text-white font-bold text-lg hover:text-brand transition">
                Transportes / Locação
            </a>

            <a href="{{ route('home') }}#localizacao" onclick="toggleMobileMenu()"
                class="text-gray-900 dark:text-white font-bold text-lg hover:text-brand transition">
                Contato / Localização
            </a>
